...
def gen_dialogue_turn(clear = False):
    ...
    teacher_pairs = redis.smembers("teacherOf")
    if teacher_pairs:
        formatted = [
            f"{t.split('|')[0]} teaches {t.split('|')[1]}"
            for t in teacher_pairs
        ]
    ...
    conf.chat_history.append({
        'role': 'system',
        'content': (
            "AVAILABLE ENTITY IDS (use only these) AND RELATIONSHIPS:\n"
        ...
        "content": f"""
            ...
            {[
            {
                i: {
                    ...
                    "constraints": ops[i]['preconditions']['constraints']
                }
            }
            for i in ops
            ]}
            ...
            ### CONSTRAINTS ###
            - A constraint is a triple that needs to be true before the question, espressed in the form [subject, predicate, _], where '_' is a generic object
            - This means that the subject, to be selected as precondition entity, must be the subject of a relation with the same name as 'predicate'
            - Example:
                required_entities: entity: ClassA
                constraints: [entity, relA, _]
                This means that entity must be part of the subject of a relA relationship in the AVAILABLE ENTITY IDS (use only these) AND RELATIONSHIPS to be eligible for being the precondition
            - You have to use this information to correctly select the entities to use in your question
            - If no constraints are in place (constraints: {{}}), you don't need to check anything more than the required entities
    ...
    conf.chat_history.append({
        ...
        "content": f"""
            ...
            - DO NOT repeat values contained in the ALREADY USED SLOTS section

            CUSTOM DATA TYPES:
            Refer to these data types to fill the slotsmarked with non-basic data types:
            {[
                {
                    t: {
                        "base_type": conf.types[t]['type'],
                        "description": conf.types[t]['text']
                    }
                }
                for t in conf.types
            ]}
