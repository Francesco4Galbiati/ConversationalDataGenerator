...
def gen_dialogue_turn(clear = False):
    ...
    for p in ["likes", "dislikes"]:
        pairs = redis.smembers(f"{p}")
    ...
    conf.chat_history.append({
        'role': 'system',
        'content': (
            "AVAILABLE ENTITY IDS AND RELATIONS (use only these):\n"
    ...
    conf.chat_history.append({
        ...
        "content": f"""
            ...
            - A constraint is a triple that needs to be true before the question, espressed in the form [subject, predicate, object]
            - This means that the subject, to be selected as precondition entity, MUST NOT be the subject of a relation with the same name as 'predicate'
            - Example:
                required_entities: entity: ClassA
                constraints: not: [entity, relA, obj]
                This means that entity MUST NOT be the subject of a [entity, relA, obj] relationship in the AVAILABLE ENTITY IDS (use only these) AND RELATIONSHIPS to be eligible for being the precondition
