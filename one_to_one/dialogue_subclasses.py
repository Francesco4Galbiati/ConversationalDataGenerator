...
def gen_dialogue_turn(clear = False):
    ...
    conf.chat_history.append({
        ...
        "content": f"""
            ...
            ### CONSTRAINTS ###
            - A constraint is a triple that needs to be true before the question
            - A 'type' constraint (one with 'type' as predicate) means that the required entity mentioned as subject must ALSO be of the type mentioned as object
            - Example:
                required_entities: entity: ClassA
                constraints: [entity, type, ClassB]
                This means that entity must be of type ClassA AND of type ClassB
            - You have to use this information to correctly select the entities to use in your question
            - If no constraints are in place (constraints: {{}}), you don't need to check anything more than the required entitiess
    ...
    for slot in ops[intent]['postconditions']['slots']:
        ...
        if slot in conf.subclasses:
            slots.extend(sorted(redis.smembers(f"entities:{conf.subclasses[slot]}")))
        if slots:
            entities_text.append(
    ...
    conf.chat_history.append({
        ...
        "content": f"""
            ...
            ### INPUT ###
            - Question: {question}
            - Conversation history (contains already used entity IDs and state)
